 ...
 * All rights reserved.
 * <p>
 * Implements the functionality of chance cards
...
public class ChanceCardHelper {
    ...
    static void chanceCard2(Game game) {
        ...
        ItemGainedOrLost.display(new ArrayList<>(cards), true, currentPlayer.getName());
    }
    ...
    static void chanceCard3(Game game) {
        ...
        game.moveShip(ship, mudBay.getPosition());
    ...
    static void chanceCard5(Game game) {
        ...
        game.moveShip(ship, homePort.getLocation());
    ...
    static void chanceCard6(Game game) {
        ...
        game.moveShip(ship, newPosition);
    ...
    static void chanceCard7(Game game) {
        ...
        if (player.getPlayerShip().getNumOfTreasures() != 0 && other.getPlayerShip().freeSpace() != 0) {
            ...
            ItemGainedOrLost.display(new ArrayList<>(Collections.singletonList(treasure)),
    ...
    static void chanceCard8(Game game) {
        ...
        if (player.getPlayerShip().getNumOfTreasures() != 0) {
            ...
            ItemGainedOrLost.display(
                    new ArrayList<>(Collections.singletonList(treasure)),
                    false, player.getName());
        } else {
            ArrayList<CrewCard> cards = player.getCrewCards();
            ArrayList<Receivable> cardsLost = new ArrayList<>();
    ...
    static void chanceCard9(Game game) {
        ...
        if (player.getPlayerShip().getNumOfTreasures() != 0) {
            ...
            if (card != null) {
                sendCrewCardToFlatIsland(game, card);
                ItemGainedOrLost.display(
                        new ArrayList<>(Collections.singletonList(card)),
                        false, player.getName());
            }
        }
    }
    ...
    static void chanceCard11(Game game) {
        takeTreasureOrCrew(game, 5, 2);
    }

    static void chanceCard12(Game game) {
        takeTreasureOrCrew(game, 4, 2);
    }

    static void chanceCard13(Game game) {
        takeTreasureOrCrew(game, 5, 2);
    }

    static void chanceCard14(Game game) {
        takeTreasureOrCrew(game, 7, 3);
    }

    static void chanceCard15(Game game) { //Take 2 chance cards
        takeCrewCards(game, 2);
    }

    static void chanceCard16(Game game) {
        SelectTreasure.display(
                7,
                game.getCurrentPlayer().getPlayerShip().freeSpace(),
                game.getGameBoard().getTreasureIsland().getTreasures(),
                game.getCurrentPlayer().getPlayerShip());
        reduceCrewCardToValue(10, game.getCurrentPlayer(), game);
    }

    static void chanceCard17(Game game) {
        SelectTreasure.display(
                6,
                game.getCurrentPlayer().getPlayerShip().freeSpace(),
                game.getGameBoard().getTreasureIsland().getTreasures(),
                game.getCurrentPlayer().getPlayerShip());
        reduceCrewCardToValue(11, game.getCurrentPlayer(), game);
    }

    static void chanceCard18(Game game) {
        SelectTreasure.display(4,
                game.getCurrentPlayer().getPlayerShip().freeSpace(),
                game.getGameBoard().getTreasureIsland().getTreasures(),
                game.getCurrentPlayer().getPlayerShip());
        if (game.getCurrentPlayer().getMoveStrength() < 7) {
            takeCrewCards(game, 2);
        }
    }

    static void chanceCard19(Game game) {
        int noOfCards = game.getCurrentPlayer().getCrewCards().size();

        Iterator<CrewCard> it = game.getCurrentPlayer().getCrewCards().iterator();
        while (it.hasNext()) {
            CrewCard c = it.next();
            it.remove();
            game.getGameBoard().getPirateIsland().returnCrewCard(c);
        }

        for (int i = 0; i < noOfCards; i++) {
            game.getCurrentPlayer().getCrewCards().add(game.getGameBoard().getPirateIsland().getTopCard());
        }

        ItemGainedOrLost.display(new ArrayList<>(game.getCurrentPlayer().getCrewCards()),
                true, game.getCurrentPlayer().getName());
    }

    static void chanceCard20(Game game) {
        ArrayList<Player> players = getOtherPlayersAtTreasureIsland(game);
        if (players.size() == 0) { //They is noone else at treasure island
            ArrayList<Receivable> cardsLost = new ArrayList<>();

            for (int i = 0; i < 2; i++) {
                CrewCard card = game.getCurrentPlayer().removeSingleCrewCard();
                game.getGameBoard().getPirateIsland().returnCrewCard(card);
                cardsLost.add(card);
            }

            ItemGainedOrLost.display(cardsLost, false, game.getCurrentPlayer().getName());
        } else { //They is one other person
            Player player = game.getCurrentPlayer();
            if (players.size() == 1) {
                Player other = players.get(0);
            } else {
                Player other = chooseOtherPlayer(game);
            }
            Player other = players.get(0);
    ...
    public static void chanceCard25And26(Game game) {
        SelectTreasure.display(
                7,
                game.getCurrentPlayer().getPlayerShip().freeSpace(),
                game.getGameBoard().getTreasureIsland().getTreasures(),
    ...
    static void chanceCard27(Game game) {
        takeTreasureOrCrew(game, 5, 3);
    }

    static void chanceCard28(Game game) {
        takeCrewCards(game, 2);
    }
    ...
    private static ArrayList<CrewCard> loseNumOfCrewCards(buccaneer.main.Player player, int numOfCards) {
        ...
        for (int i = 0; i < numOfCards; i++) {
            if (player.getCrewCards().size() > 0) {
                ...
                ErrorMessage.display("Haha - you can't take any more of my crew!" +
                        " I don't have any crew left! Hahahaha ohh wait...");
                break;
            }
        }
        return cards;
    ...
    private static ArrayList<Player> getOtherPlayersAtTreasureIsland(Game game) {
        ...
        players.removeIf(player ->
                !player.getPlayerShip().getLocation().isNextToOrOnIsland(treasureIsland));
        return players;
    }
    ...
    private static void reduceCrewCardToValue(int value, Player p, Game g) {
        int pStrength = p.getMoveStrength();
        p.getCrewCards().sort(Comparator.comparing(CrewCard::getValue).reversed());
        Iterator<CrewCard> it = p.getCrewCards().iterator();
        ArrayList<Receivable> cardsLost = new ArrayList<>();
        while (it.hasNext() && pStrength > value) {
            CrewCard c = it.next();
            if (pStrength - c.getValue() <= value) {
                it.remove();
                p.removeCrewCard(c);
                g.getGameBoard().getPirateIsland().returnCrewCard(c);
                cardsLost.add(c);
                break;
            } else if (pStrength - c.getValue() > value) {
                it.remove();
                p.removeCrewCard(c);
                g.getGameBoard().getPirateIsland().returnCrewCard(c);
                cardsLost.add(c);
            } else if (value < pStrength) {
                break;
            }
            pStrength = p.getMoveStrength();
        }
    ...
    private static void takeCrewCards(Game g, int crew) {
        ...
        for (int i = 0; i < crew; i++) {
            if (g.getGameBoard().getPirateIsland().getCrewCardDeck().getSize() == 0) {
                ErrorMessage.display("Shiver me timbers - It appears they's no more crew cards at"+
                        " Pirate Island!");
                break;
            } else {
